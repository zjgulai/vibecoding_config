...
完成标准：Spec/Standards 分开；安全 finding 有可利用性证据；AI Eval 可复现；自动扫描不是完整质量证明；blocker 未关闭时明确 NO-GO。
```
